...
export function createEstimate(estimate, errors=null) {
    return dispatch => {
        ...
        axios.post(ENDPOINT_ESTIMATE, estimate)
            .then(function(response) {
                dispatch(createEstimateSuccess(response.data))
            }).catch(function(error) {
                dispatch(createEstimateFailed(error.response?error.response.data:null))
            });
    }
}
...
export function retrieveEstimate(id) {
    return dispatch => {
        ...
        axios.get(ENDPOINT_ESTIMATE + id + '/')
            .then(function(response) {
                dispatch(retrieveEstimateSuccess(response.data))
            }).catch(function(error) {
                dispatch(retrieveEstimateFailed(error.response?error.response.data:null))
            });
    }
}
...
export function updateEstimate(id, data) {
    return dispatch => {
        ...
        axios.patch(ENDPOINT_ESTIMATE + id + '/', data)
            .then(function(response) {
                dispatch(updateEstimateSuccess(response.data))
            }).catch(function(error) {
                dispatch(updateEstimateFailed(error.response?error.response.data:null))
            });
    }
}
...
export function deleteEstimate(id) {
    return dispatch => {
        ...
        axios.delete(ENDPOINT_ESTIMATE + id + '/', {})
            .then(function() {
                dispatch(deleteEstimateSuccess(id))
            }).catch(function(error) {
                dispatch(deleteEstimateFailed(error.response?error.response.data:null))
            });
    }
}
