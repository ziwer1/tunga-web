...
export function createQuote(quote, errors=null) {
    return dispatch => {
        ...
        axios.post(ENDPOINT_QUOTE, quote)
            .then(function(response) {
                dispatch(createQuoteSuccess(response.data))
            }).catch(function(error) {
                dispatch(createQuoteFailed(error.response?error.response.data:null))
            });
    }
}
...
export function createQuoteSuccess(quote) {
    sendGAEvent(GA_EVENT_CATEGORIES.TASK, GA_EVENT_ACTIONS.APPLY, getGAUserType(getUser()));
    return {
...
export function listQuotes(filter) {
    return dispatch => {
        ...
        axios.get(ENDPOINT_QUOTE, {params: filter})
            .then(function(response) {
                dispatch(listQuotesSuccess(response.data))
            }).catch(function(error) {
                dispatch(listQuotesFailed(error.response?error.response.data:null))
            });
    }
}
...
export function retrieveQuote(id) {
    return dispatch => {
        ...
        axios.get(ENDPOINT_QUOTE + id + '/')
            .then(function(response) {
                dispatch(retrieveQuoteSuccess(response.data))
            }).catch(function(error) {
                dispatch(retrieveQuoteFailed(error.response?error.response.data:null))
            });
    }
}
...
export function updateQuote(id, data) {
    return dispatch => {
        ...
        axios.patch(ENDPOINT_QUOTE + id + '/', data)
            .then(function(response) {
                dispatch(updateQuoteSuccess(response.data))
            }).catch(function(error) {
                dispatch(updateQuoteFailed(error.response?error.response.data:null))
            });
    }
}
...
export function deleteQuote(id) {
    return dispatch => {
        ...
        axios.delete(ENDPOINT_QUOTE + id + '/', {})
            .then(function() {
                dispatch(deleteQuoteSuccess(id))
            }).catch(function(error) {
                dispatch(deleteQuoteFailed(error.response?error.response.data:null))
            });
    }
}
