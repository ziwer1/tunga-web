...
function getNextActivityState(state, action, type, old_activity=[], new_activity=[]) {
    var item_key = null;
    switch (type) {
        case ITEM_TYPE_TASK:
    ...
    if(item_key) {
        var item_activity = {};
        item_activity[item_key] = [...old_activity || [], ...getReversedActivity(action.items), ...new_activity || []];
...
function activity(state = {}, action) {
    switch (action.type) {
        ...
        case TaskActions.LIST_MORE_TASK_ACTIVITY_SUCCESS:
            if(action.items) {
                return getNextActivityState(state, action, ITEM_TYPE_TASK, [], state[getTaskKey(action.id)]);
            }
            return state;
        case ChannelActions.LIST_MORE_CHANNEL_ACTIVITY_SUCCESS:
            if(action.items) {
                return getNextActivityState(state, action, ITEM_TYPE_CHANNEL, [], state[getChannelKey(action.id)]);
            }
            return state;
        case TaskActions.LIST_NEW_TASK_ACTIVITY_SUCCESS:
            return getNextActivityState(state, action, ITEM_TYPE_TASK, getConcreteActivities(state, getTaskKey(action.id)));
        case ChannelActions.LIST_NEW_CHANNEL_ACTIVITY_SUCCESS:
            return getNextActivityState(state, action, ITEM_TYPE_CHANNEL, getConcreteActivities(state, getChannelKey(action.id)));
        case CREATE_COMMENT_SUCCESS:
            return getNextStateOptimistic(
                state,
                [{action: 'comment', activity_type: 'comment', activity: action.comment}],
