...
export default class TaskDocument extends FormComponent {
  ...
  handleSubmit(e) {
    ...
    TaskActions.createTaskDocument(task.id, {file_type: this.state.file_type, description: this.state.description}, this.state.file);
  }
          ...
          : <div style={{marginTop: '20px'}}>
            <form
              ...
              Task.detail.error.document.file_type
                ? <FieldError
                  message={Task.detail.error.document.file_type}
                />
                : null}
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={this.onInputChange.bind(this, 'file_type')}
                  required>
                  <option value="1">- Select -</option>
                  {Object.keys(TASK_DOCUMENT_TYPES).map(file_type => {
                    return (
                      <option value={file_type}>{TASK_DOCUMENT_TYPES[file_type]}</option>
                    )
                  })}
                </select>
              </div>
              ...
              Task.detail.error.document.file
                ...
                <div className="form-group">
                  ...
                  <div>
                    ...
                    {this.state.file
                      ? <div>
                        <i className="fa fa-file-text-o" /> {this.state.file.name}
                      </div>
                      : null}
                    <button
                      type="button"
              ...
              Task.detail.error.document.description
                ? <FieldError
                  message={Task.detail.error.document.description}
                />
                : null}
              <div className="form-group">
                <label className="control-label">
