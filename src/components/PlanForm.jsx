...
export default class PlanForm extends React.Component {
    ...
    componentDidMount() {
        ...
        if(activity) {
            this.setState({start_date: activity.start_date, end_date: activity.end_date});
        }
    }
    ...
    handleSubmit(e) {
        ...
        if(this.props.onSave) {
            this.props.onSave({...this.props.activity, title, start_date, end_date, description});
        }
        if(this.props.close) {
            this.props.close();
        }
    ...
    render() {
        ...
        return (
            <div>
                <form onSubmit={this.handleSubmit} name="activity" role="form" ref="activity_form">
                    {(this.state.error && this.state.error.title)?
                        (<FieldError message={this.state.error.title}/>):null}
                    <div className="form-group">
                        <label className="control-label">Title *</label>
                        <div><input type="text" className="form-control" ref="title" required placeholder="Title" defaultValue={activity.title}/></div>
                    </div>
                    ...
                    {(this.state.error && this.state.error.description)?
                        (<FieldError message={this.state.error.description}/>):null}
                    <div className="form-group">
                    ...
                    <div className="text-center">
                        <button type="submit" className="btn  ">{activity.idx > -1?'Update':'Add'} Milestone</button>
                    </div>
                </form>
            </div>
        );
