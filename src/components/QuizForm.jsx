...
export default class QuizForm extends React.Component {
  ...
  renderHeaderContent() {
    ...
    sections = [
      {
        ...
        items: [<StartQuiz onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
      {
        title: 'What kind of product are you building?',
        items: [<TaskType type={this.state.type} onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
      {
        title: 'What is the scope for your project?',
        items: [<TaskScope scope={this.state.scope} onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
      {
        title: 'Do you know which technologies you want to use?',
        items: [<TaskTech onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
      {
        title: 'Do you have detailed project specifications?',
        items: [<TaskSpecification onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
      {
        title: 'Do you need help in managing and cordinating the project?',
        items: [<TaskManage onStateValueChange={(stage) => this.onStateValueChange(stage)} />],
      },
    ];
    ...
    return (
      <ContentWrapper current_section={current_section}/>
    );
  }
  ...
  render() {
    ...
    return (
      <ShowcaseContainer
        className={(this.state.autoSubmit)? 'quiz-page-result' : 'quiz-page quiz'}
                ...
                return (
                    <div key={scope_type.id} className="choice">
                        <button
                            type="button"
                            className={
                                'btn' + (props.scope == scope_type.id ? ' active' : '')
                            }
