...
export default class ProfileForm extends React.Component {
    ...
    showAll(e) {
        this.setState({showAll: !this.refs.profile_form.checkValidity(), overrideErrors: false});
    }
    ...
    handleSubmit(e) {
        ...
        req_data.company = this.refs.company?this.refs.company.value.trim():null;
        req_data.vat_number = this.refs.vat_number?this.refs.vat_number.value.trim():profile.vat_number;
        req_data.country = this.refs.country?this.refs.country.value.trim():null;
        ...
        req_data.plot_number = this.refs.plot_number?this.refs.plot_number.value.trim():null;
        req_data.postal_code = this.refs.postal_code?this.refs.postal_code.value.trim():null;
        req_data.phone_number = this.refs.phone_number?this.refs.phone_number.value.trim():null;
        req_data.id_document = this.state.photo;
        req_data.payment_method = this.state.payment_method;
        req_data.mobile_money_cc = this.state.country_code || profile.mobile_money_cc;
        req_data.mobile_money_number = this.refs.mobile_money_number?this.refs.mobile_money_number.value.trim():profile.mobile_money_number;
        req_data.btc_address = this.refs.btc_address?this.refs.btc_address.value.trim():profile.btc_address;
    ...
    render() {
        ...
        let personalComp = (
            <div>
                {(Profile.error.profile && Profile.error.profile.first_name)?
                    (<FieldError message={Profile.error.profile.first_name}/>):null}
                ...
                {(Profile.error.profile && Profile.error.profile.last_name)?
                    (<FieldError message={Profile.error.profile.last_name}/>):null}
                <div className="form-group">
        ...
        let addressComp = (
            <div>
                {(Profile.error.profile && Profile.error.profile.country)?
                    (<FieldError message={Profile.error.profile.country}/>):null}
                <div className="form-group">
                        ...
                        <div>
                            {Profile.profile.payment_method == PAYMENT_METHOD_BTC_WALLET && Profile.profile.btc_wallet && Profile.profile.btc_wallet.provider == 'coinbase'?(
                                ...
                                <a href={SOCIAL_LOGIN_URLS.coinbase + `?action=connect&next=/profile/payment/coinbase/`}
                                   className="btn coinbase-connect-button" title="Connect with Coinbase" target="_blank">
        ...
        let companyComp = (
            <div>
                ...
                <div className="form-group">
                    <label className="control-label">Company {(Auth.user.is_developer || Auth.user.is_project_manager)?'(if applicable)':null}</label>
                    <div><input type="text" className="form-control" ref="company" placeholder="Company" defaultValue={Profile.profile.company}/></div>
                </div>

                {((Auth.user.is_developer || Auth.user.is_project_manager) && Profile.error.profile && Profile.error.profile.vat_number)?
        ...
        let IDComp = (
            <div>
                {(Profile.error.profile && Profile.error.profile.id_document)?
                    (<FieldError message={Profile.error.profile.id_document}/>):null}
                <div className="form-group">
        ...
        return (
            <div>
                ...
                <div className="form-wrapper profile-form">
                    {current_section && current_section.title && !canShowAll?(
                        <div className="section-title clearfix">
                            <h4 className="pull-left"
                                id="profile-wizard-title">{current_section.title}</h4>
                    ...
                    <form onSubmit={this.handleSubmit.bind(this)} name="profile" role="form" ref="profile_form" className={canShowAll?'steps-all':null}>
